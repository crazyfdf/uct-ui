...
export default function uniLogin () {
  uni.login({
    ...
    univerifyStyle: { // 自定义登录框样式
      ...
      "privacyTerms": {
        ...
        "privacyItems": [
          ...
          {
            "url": "https://m.capwhale.com/index/userTerms.html", // 点击跳转的协议详情页面  
            "title": "用户服务协议" // 协议名称  
          },
          {
            "url": "https://m.capwhale.com/index/privacy.html", // 点击跳转的协议详情页面  
            "title": "隐私政策" // 协议名称  
          }
        ]
      }
    ...
    success (res) { // 登录成功
      ...
      api("login.loginByUniverify", {
        accesstoken: access_token,
        openid,
      }).then((res2) => {
        if (res2.code == "000")
        {
          uni.$tools.showToast('登录成功')
          console.log(res2);
          store.dispatch('getUserInfo')
          store.dispatch('setTokenAndBack', true)
        }
      });
    },
    fail (res) {  // 登录失败
      console.log("univerify登录失败", res)
