<template>
	<view>
		...
		>
			<!-- Form布局组件 -->
			<!-- card布局 -->
			<uct-card v-if="item.type == 'card'" :className="['bc-gray']" :title="item.label">
				<uct-form
					@dataItem="dataItem"
					slot="body"
			...
			<!-- 分割线 -->
			<uct-divider v-if="item.type == 'divider'" :item="item"></uct-divider>
			<!-- Form布局组件END -->
			...
			></uct-title>
			<!-- 警告提示 -->
			<uct-alert v-if="item.type == 'alert'" :item="item"></uct-alert>
			<!-- HTML -->
			<rich-text v-if="item.type == 'html'" :nodes="item.options.defaultValue"></rich-text>
			<!-- 按钮 -->
			<uct-button
